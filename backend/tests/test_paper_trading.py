...
def _cap_account(tmp_path, cash: float = 1_000_000.0) -> dict:
    return paper.create_account(cash)
...
def _write_factor(tmp_path, day: date, factor: float, asset_type: str = "stock") -> None:
    ...
    pl.DataFrame(
        ...
        schema={"symbol": pl.String, "trade_date": pl.Date, "ex_factor": pl.Float64}).write_parquet(out)
...
def test_create_order_requires_account(tmp_path):
    with pytest.raises(ValueError, match="尚未创建"):
        paper.create_order(SYM, "buy", qty=100)


def test_create_order_rejects_bad_qty_and_insufficient_cash(tmp_path):
    _cap_account(tmp_path, 100_000.0)
    _, err = paper.create_order(SYM, "buy", qty=150)  # 非百股整数倍
    assert "100 的整数倍" in err
    _, err = paper.create_order(SYM, "buy", qty=2000, ref_price=100.0)
    assert "资金不足" in err
...
def test_create_order_etf_market_converts_to_next_open(tmp_path):
    ...
    order, err = paper.create_order(
        "510300.SH", "buy", qty=1000, order_type="market", asset_type="etf", ref_price=4.0)
    assert err is None
...
def test_sell_requires_holding_and_t1(tmp_path, monkeypatch):
    ...
    _cap_account(tmp_path)
    _, err = paper.create_order(SYM, "sell", qty=100)
    assert "不能卖出" in err

    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    buy_order, err = paper.create_order(SYM, "buy", qty=100, ref_price=10.0)
    assert err is None
...
def test_cancel_pending_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(paper, "cn_today", lambda: day)
    _cap_account(tmp_path)
    order, _ = paper.create_order(SYM, "buy", qty=100, ref_price=10.0)
    cancelled, err = paper.cancel_order(order["id"])
    assert err is None and cancelled["status"] == "cancelled"
    _, err = paper.cancel_order(order["id"])
    assert "不可撤销" in err
...
def test_immediate_fill_cash_and_positions_consistent(tmp_path, monkeypatch):
    ...
    order, err = paper.create_order(SYM, "buy", qty=1000, ref_price=10.0)
    assert err is None
    ...
    filled = paper.get_order(order["id"])
    # 滑点向上: 10 * 1.0005
    ...
    positions, cash = paper.replay_positions()
...
def test_limit_down_sell_rejected(tmp_path, monkeypatch):
    ...
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    _cap_account(tmp_path)
    _, err = paper.create_order(SYM, "buy", qty=100, ref_price=10.0)
    assert err is None
...
def test_settle_next_open_close_and_postpone_expire(tmp_path):
    ...
    no_open, _ = paper.create_order(SYM, "buy", qty=100, order_type="next_open", ref_price=10.0)
    close_ord, _ = paper.create_order(SYM, "buy", qty=100, order_type="close", ref_price=10.0)
    summary = paper.settle_day(tmp_path, day.isoformat())
    assert summary["filled"] == 2
    assert paper.get_order(no_open["id"])["fill_price"] == pytest.approx(10.2 * 1.0005)
    assert paper.get_order(close_ord["id"])["fill_price"] == pytest.approx(10.8 * 1.0005)

    # 停牌顺延 → 连续无行情 4 日过期 (MAX_POSTPONE_DAYS=3)
    stuck, _ = paper.create_order(SYM, "buy", qty=100, order_type="next_open", ref_price=11.0)
    for i in range(4):
        s = paper.settle_day(tmp_path, (day + timedelta(days=i + 1)).isoformat())
    got = paper.get_order(stuck["id"])
...
def test_pending_sell_occupies_available_quota(tmp_path, monkeypatch):
    ...
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    _cap_account(tmp_path)
    _, err = paper.create_order(SYM, "buy", qty=200, ref_price=10.0)
    assert err is None
    assert len(paper.evaluate_intraday(tmp_path, {SYM: 10.0})) == 1
    # 次日: 可卖 200
    monkeypatch.setattr(paper, "cn_today", lambda: day + timedelta(days=1))
    ...
    _, err = paper.create_order(SYM, "sell", qty=200)
    assert err is not None and ("可卖" in err or "占用" in err)
    # 撤单后额度释放, 可以再挂
    paper.cancel_order(s1["id"])
    s2, err = paper.create_order(SYM, "sell", qty=100)
...
def test_pending_buy_occupies_cash_at_fill(tmp_path, monkeypatch):
    ...
    a, _ = paper.create_order(SYM, "buy", qty=500, order_type="close", ref_price=10.0)
    b, err = paper.create_order(SYM, "buy", qty=500, order_type="close", ref_price=10.0)
    assert err is None  # 预检按单张各自通过, 由撮合侧 (资金/占用双闸) 兜底
    paper.settle_day(tmp_path, day.isoformat())
    assert paper.get_order(a["id"])["status"] == "filled"
    got_b = paper.get_order(b["id"])
    # b 必须被拒且留痕: 现金已被 a 消耗, 基础资金校验或占用检查任一拦下都算防线生效
    assert got_b["status"] == "expired"
    assert got_b["reason"] is not None
    # 台账只有一张成交, 现金不透支
    fills = [f for f in paper.load_fills() if f.get("kind", "fill") == "fill"]
    assert len(fills) == 1
    _, cash = paper.replay_positions()
    assert cash >= 0
...
def test_position_symbol_cap(tmp_path, monkeypatch):
    ...
    assert "上限" in err
    # 加仓已有持仓 → 放行
    ok, err = paper.create_order("600000.SH", "buy", qty=100, ref_price=10.0)
...
def test_settle_idempotent_no_double_fill(tmp_path):
    ...
    paper.create_order(SYM, "buy", qty=100, order_type="next_open", ref_price=10.0)
    paper.settle_day(tmp_path, day.isoformat())
...
def test_corporate_action_adjusts_position_and_idempotent(tmp_path):
    ...
    order, _ = paper.create_order(SYM, "buy", qty=1000, order_type="close", ref_price=10.0)
    paper.settle_day(tmp_path, (day - timedelta(days=1)).isoformat())
    assert paper.load_positions()[SYM]["qty"] == 1000

    _write_factor(tmp_path, day, 1.25)
    paper.settle_day(tmp_path, day.isoformat())
    pos = paper.load_positions()[SYM]
    assert pos["qty"] == pytest.approx(1250)
    # 成本被因子摊薄: 总成本不变 (买入费已摊入), 数量乘 1.25 后 avg_cost 除以 1.25
    order_after = paper.get_order(order["id"])
    total_cost = 1000 * order_after["fill_price"] + order_after["fees"]
    replayed = paper.replay_positions()[0][SYM]
    assert replayed["avg_cost"] * 1250 == pytest.approx(total_cost, rel=1e-4)

    # 重跑同日: 不二次乘因子
    paper.settle_day(tmp_path, day.isoformat())
    assert paper.load_positions()[SYM]["qty"] == pytest.approx(1250)
    corp = [f for f in paper.load_fills() if f.get("kind") == "corp_action"]
    assert len(corp) == 1
...
def test_nav_and_overview_math(tmp_path, monkeypatch):
    ...
    _cap_account(tmp_path, 100_000.0)
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    _, err = paper.create_order(SYM, "buy", qty=1000, ref_price=10.0)
    assert err is None
    paper.evaluate_intraday(tmp_path, {SYM: 10.0})
    fill = next(f for f in paper.load_fills() if f.get("kind", "fill") == "fill")

    # 盘中估算: 价格 11 → 市值 11000
    ov = paper.overview({SYM: 11.0})
...
def test_auto_rule_crud_and_validation(tmp_path):
    ...
    with pytest.raises(ValueError, match="不能为空"):
        paper_auto.create_auto_rule(_auto_rule(name=""))
    with pytest.raises(ValueError, match="match_kind"):
        paper_auto.create_auto_rule(_auto_rule(match_kind="xxx"))
    with pytest.raises(ValueError, match="不能超过 100"):
        paper_auto.create_auto_rule(_auto_rule(size_mode="pct_equity", size_value=150))
    rule = paper_auto.create_auto_rule(_auto_rule())
    assert rule["id"].startswith("arule_")
    assert len(paper_auto.load_auto_rules()) == 1
    paper_auto.set_enabled(rule["id"], False)
    assert paper_auto.load_auto_rules(enabled_only=True) == []
    assert paper_auto.delete_auto_rule(rule["id"]) is True
    assert paper_auto.delete_auto_rule(rule["id"]) is False
...
def test_auto_trigger_matches_strategy_event(tmp_path, monkeypatch):
    ...
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    _cap_account(tmp_path)
    paper_auto.create_auto_rule(_auto_rule())

    ev = {"source": "strategy", "strategy_id": "strat_1", "rule_id": "r1",
          "symbol": SYM, "price": 10.0}
    orders = paper_auto.on_rule_events([ev])
    assert len(orders) == 1
    ...
    # 冷却期内同 symbol 不再触发
    assert paper_auto.on_rule_events([ev]) == []
    # 冷却外 (6 天后) 恢复触发
    paper_auto._now_iso = lambda: ""  # no-op 防误用提示
    rule_id = orders[0]["source"].split(":", 1)[1]
    paper_auto.delete_auto_rule(rule_id) if False else None
    # 直接改订单 created_at 模拟 6 天前
    orders_loaded = paper.load_orders()
    for o in orders_loaded:
        o["created_at"] = "2026-09-18T10:00:00+08:00"
        paper.save_order(o)
    orders2 = paper_auto.on_rule_events([ev])
...
def test_auto_trigger_non_matching_events_ignored(tmp_path):
    ...
    assert paper_auto.on_rule_events([ev1, ev2, ev3]) == []


def test_auto_rule_disabled_not_triggered(tmp_path):
    from app.strategy import paper_auto
    _cap_account(tmp_path)
    rule = paper_auto.create_auto_rule(_auto_rule(enabled=False))
    ev = {"source": "strategy", "strategy_id": "strat_1", "symbol": SYM, "price": 10.0}
    assert paper_auto.on_rule_events([ev]) == []
    paper_auto.set_enabled(rule["id"], True)
    assert len(paper_auto.on_rule_events([ev])) == 1
...
def test_limit_queue_retries_next_open_then_expires(tmp_path, monkeypatch):
    ...
    day = date(2026, 9, 24)
    monkeypatch.setattr(paper, "cn_today", lambda: day)
    paper.create_account(1_000_000, queue_limit_orders=True)
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    order, _ = paper.create_order(SYM, "buy", qty=100, ref_price=10.0)
...
def test_limit_queue_fills_when_open_below_limit(tmp_path, monkeypatch):
    ...
    assert paper.get_order(order["id"])["status"] == "pending"
    summary = paper.settle_day(tmp_path, day.isoformat())
...
def test_multi_account_isolation(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(paper, "cn_today", lambda: day)
    _write_daily(tmp_path, [(day - timedelta(days=1), 10.0, 10.0)])
    paper.create_account(1_000_000, account_id="default", name="主账户")
    paper.create_account(500_000, account_id="acc_a", name="策略A")

    # 同一标的分别在两个账户各买一笔
    assert paper.evaluate_intraday(tmp_path, {SYM: 10.0}, account_id="default") == []  # 无订单
    o1, err = paper.create_order(SYM, "buy", qty=100, account_id="default")
    o2, err = paper.create_order(SYM, "buy", qty=200, account_id="acc_a")
    assert err is None
    ...
    # 订单互不可见
    assert [o["id"] for o in paper.load_orders("default")] == [o1["id"]]
    assert [o["id"] for o in paper.load_orders("acc_a")] == [o2["id"]]
    # 持仓互不可见
    assert paper.load_positions("default")[SYM]["qty"] == 100
    assert paper.load_positions("acc_a")[SYM]["qty"] == 200
    # 现金互不可见 (含费用扣减不同)
    ov1 = paper.overview(account_id="default")
    ov2 = paper.overview(account_id="acc_a")
    assert ov1["cash"] > ov2["cash"]
    assert ov1["account_name"] == "主账户" and ov2["account_name"] == "策略A"

    # 账户列表与遍历
    assert paper.list_account_ids() == ["default", "acc_a"]
    infos = {a["id"]: a for a in paper.list_accounts()}
    assert set(infos) == {"default", "acc_a"} and infos["acc_a"]["name"] == "策略A"

    # settings 只影响目标账户
    paper.update_settings("acc_a", queue_limit_orders=True)
    assert paper.get_account("default")["queue_limit_orders"] is False
    assert paper.get_account("acc_a")["queue_limit_orders"] is True

    # 净值隔离
    paper.settle_day(tmp_path, day.isoformat(), account_id="default")
    paper.settle_day(tmp_path, day.isoformat(), account_id="acc_a")
    nav1 = paper.load_nav("default")
...
def test_day_fill_events_returns_todays_fills(tmp_path, monkeypatch):
    ...
    # 当日无成交 → 空列表
    assert paper.day_fill_events(day.isoformat()) == []

    _, err = paper.create_order(SYM, "buy", qty=1000, ref_price=10.0)
    assert err is None
    assert len(paper.evaluate_intraday(tmp_path, {SYM: 10.0})) == 1

    events = paper.day_fill_events(day.isoformat())
    assert len(events) == 1
    ev = events[0]
    assert ev["source"] == "paper" and ev["type"] == "fill" and ev["severity"] == "info"
    assert ev["symbol"] == SYM and ev["side"] == "buy" and ev["qty"] == 1000
    ...
    # 幂等 (结算重跑同日不重复追加事件)
    assert paper.day_fill_events(day.isoformat()) == events
